...
function Timeline( clip, bone_name, type = "skeleton" ) {
	...
	if(this.type == "skeleton")
		this.processSkeletonTracks();
	else
		this.processBlendshapesTracks();
	...
	this.onDrawContent = ( ctx, time_start, time_end, timeline ) => {
		...
		let tracks = this.tracksPerBone[this.selected_bone] || this.tracksPerBone[this.getTrackName(this.selected_bone)[0]];
		if(!tracks) return;
...
Timeline.prototype.processSkeletonTracks = function () {
...
// Creates a map for each BS -> tracks
Timeline.prototype.processBlendshapesTracks = function () {
...
// Timeline.prototype.setSelectedMorphTarget = function ( morph_name ) {

// 	if(morph_name.constructor !== String)
// 	throw("Morph target name has to be a string!");

// 	this.selected_moprph = morph_name;
// 	this.unSelectAllKeyFrames();
// }
