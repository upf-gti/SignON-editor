...
{
    if(data.multiFaceLandmarks.length<1)
        return;
    //var multiFaceLandmarks = Object.assign({}, data.multiFaceLandmarks[0]);
    const scaledMesh = data.multiFaceLandmarks[0];
    var [distanceX, distanceY, distanceZ] = [scaledMesh["168"].x, scaledMesh["168"].y, scaledMesh["168"].z];
    var center_scaledMesh = scaledMesh;

    for (var i = 0; i < scaledMesh.length; ++i) {
       /* multiFaceLandmarks[j].x = (multiFaceLandmarks[j].x - 0.5);
        multiFaceLandmarks[j].y = (1.0 - multiFaceLandmarks[j].y) + 2;
        multiFaceLandmarks[j].z = multiFaceLandmarks[j].z * 0.5;*/
        const [x, y, z] = [scaledMesh[i].x, scaledMesh[i].y, scaledMesh[i].z];
        var x_final = (-x + distanceX);
        var y_final = (-y + distanceY);
        var z_final = (-z + distanceZ);
        var v = [x_final, y_final, z_final];
        center_scaledMesh[i] = v;
        scaledMesh[i].x = x_final;
        scaledMesh[i].y = y_final;
        scaledMesh[i].z = z_final;
    }

    project.landmarks.push({FLM: scaledMesh, dt: _dt, scale: distanceZ});
}
